...
from smooth import removeOuterPoint, detectEvent, smoothByEvent, detectBallTypeByEvent, removeMotionOutliers
from LayerContent.CES.BallInfo import getBallInfo, writeBallInfo
from SmashBallInfo import runSmashAnalyze, writeSmashBallInfo
from smooth_gradient_minimize import smoothByEvent_gradient_minimize
from LayerContent.tools.draw import drawTrajectory, drawGradientTrajectory, drawVisualization

from lib.common import  loadConfig, loadNodeConfig
from lib.inspector import sendNodeStateMsg
from lib.point import Point, load_points_from_csv, save_points_to_csv
...
def triangluation(config, output_csv=None, atleast1hit=False):
    ...
    while len(all_points_2d) > 0:
        ...
        if undistort_points_2D.shape[0] >= 2:
            ...
            # print('-->>point', point3d.fid, point3d.y, point3d.timestamp)
            # print()
...
def main():
    ...
    removeMotionOutliers(date) # Output：rm
    detectEvent(date) # Output：event_1
    drawTrajectory(date)
    print()
    print('===== CES demo =====')
    gradient_is_success = smoothByEvent_gradient_minimize(date, fps = fps)
    if gradient_is_success:
        drawGradientTrajectory(date)
    drawVisualization(date)
    ball_info = runSmashAnalyze(date, gradient_is_success)
    print(ball_info)
    writeSmashBallInfo(date, ball_info)
