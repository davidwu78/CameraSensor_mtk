...
from LayerContent.tools.draw import drawPoints
...
def triangluation(config, output_csv=None, atleast1hit=False):
    ...
    for c in cameras:
        ...
        metapath = None
        file_name = f"{c}_meta.csv"
        if os.path.exists(os.path.join(folder, file_name)):
            metapath = os.path.join(folder, file_name)

        if os.path.exists(os.path.join(folder, mp4.split('.')[0]+'_ball.csv')): # Labeled
            points_2d = load_points_from_csv(os.path.join(folder,mp4.split('.')[0]+'_ball.csv'), metapath)
        elif c in video_csv.keys() and os.path.exists(os.path.join(folder, video_csv[c]+'.csv')): # TrackNet
            points_2d = load_points_from_csv(os.path.join(folder, video_csv[c]+'.csv'), metapath)
        else:
    ...
    while len(all_points_2d) > 0:
        ...
        # debug
        # print('---------------------------------------------------------')
        # if points_2d_triangulated:
        #     for i, point in enumerate(points_2d_triangulated, start=1):
        #         print(f'[P{i}]: cam: {point.cam_idx}, fid: {point.fid}, timestamp: {point.timestamp}, vis: {point.visibility}')
